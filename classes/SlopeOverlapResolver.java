...
public class SlopeOverlapResolver extends AbstractOverlapResolver {
	...
	private boolean processLine(final Junction j, final Line line, final boolean updatePos) {
		...
		if (!line.getContourClass().equals(LinesUtil.contour_class.cont_both_junc)) {
			if (pos == 0) {
				// If this line is already an "end_junc", upgrade it to a "both"
				if (line.getContourClass().equals(LinesUtil.contour_class.cont_end_junc)) {
				...
				if (line.getContourClass().equals(LinesUtil.contour_class.cont_start_junc)) {
					line.setContourClass(LinesUtil.contour_class.cont_both_junc);
				}
